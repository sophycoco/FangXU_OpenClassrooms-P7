...
exports.getAllComm = (req, res, next) => {
  conn.query(
    `SELECT comment.id, comment.content, comment.user_id, comment.post_id, user.username, comment.dateCreate, user.isAdmin 
    FROM comment join post on post.id = comment.post_id join user on user.id = comment.user_id where comment.post_id=? ORDER BY dateCreate DESC`,
    req.params.id,
    (error, result) => {
      if (error) {
...
exports.getOneComm = (req, res, next) => {
  conn.query("SELECT comment.id, comment.content, user_id, isAdmin  FROM comment JOIN user ON user.id = comment.user_id WHERE comment.id=? ", req.params.id, (error, result) => {
    if (error) {
